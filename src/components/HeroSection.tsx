...
const HeroSection = () => {
  ...
  const handleVideoEnd = () => {
    setFadeIn(false); // fade out current video
    setTimeout(() => {
      setCurrentVideo((prev) => (prev + 1) % videos.length);
      setFadeIn(true); // fade in next video
    }, 300);
  };
  ...
  return (
    <section className="relative min-h-[80vh] bg-gradient-to-br from-juicy-yellow/10 to-juicy-green/10 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5">
        ...
        <div
          className="absolute top-32 right-20 w-24 h-24 bg-juicy-green rounded-full animate-float"
          style={{ animationDelay: "1s" }}
        ></div>
        <div
          className="absolute bottom-20 left-1/4 w-20 h-20 bg-juicy-red rounded-full animate-float"
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8">
            <h1 className="text-5xl md:text-6xl font-raleway font-bold text-gray-800 leading-tight">
              Squeeze the Day with{" "}
              <span className="text-juicy-green">Juicy</span>{" "}
              <span className="text-juicy-yellow">Planet</span> 🍊
            </h1>
            <p className="text-xl text-gray-600 font-quicksand leading-relaxed">
              Fresh, handcrafted juices made with love by our chef. Cold-pressed
              goodness with no additives — just pure nature in every sip.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                <Link to="/shop">Shop Fresh Juices 🥤</Link>
              </Button>
              <Button
                variant="outline"
                size="lg"
          ...
          <div className="relative">
            ...
            {/* Floating Elements */}
            <div
              className="absolute -bottom-6 -left-6 w-24 h-24 bg-juicy-yellow rounded-full opacity-80 animate-float"
              style={{ animationDelay: "0.5s" }}
            ></div>
            <div
              className="absolute -top-6 -right-6 w-16 h-16 bg-juicy-green rounded-full opacity-80 animate-float"
